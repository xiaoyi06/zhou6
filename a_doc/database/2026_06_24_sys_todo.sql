...
ALTER TABLE sys_todo ADD COLUMN IF NOT EXISTS finish_time TIMESTAMP;
...
CREATE INDEX IF NOT EXISTS idx_sys_todo_user_status_finish_time
    ON sys_todo (user_id, status, finish_time);
...
COMMENT ON COLUMN sys_todo.finish_time IS '完成或取消时间；DONE/CANCELLED 按该时间做日期查询';
