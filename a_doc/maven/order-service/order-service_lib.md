...
<table border="1" cellspacing="0" cellpadding="6">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>用于 <code>oms_order</code>、<code>oms_order_pay_receipt</code> 持久化和雪花 ID 生成</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>Spring Boot parent 3.3.0</td>
      <td>compile</td>
      <td>提供订单 REST API 能力</td>
      <td>支撑 <code>/order/api/order/createPay</code>、详情查询和退款审批接口</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>compile</td>
      <td>同步调用账户服务</td>
      <td>调用 <code>account-service</code> 的现金预冻结和红字冲正接口</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>compile</td>
      <td>服务发现负载均衡</td>
      <td>让 Feign 按 <code>account-service</code> 服务名从 Nacos 选择实例</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>Spring Boot parent 3.3.0</td>
      <td>compile</td>
      <td>接入 RabbitMQ 生产链路</td>
      <td>本地事务提交后发送 <code>zhou6.account.settle</code>，回滚或取消后发送 <code>zhou6.account.unfreeze</code></td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>保存订单主表 <code>oms_order</code> 和订单现金扣减凭证 <code>oms_order_pay_receipt</code></td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>compile</td>
      <td>读取登录会话快照</td>
      <td>业务服务按 accessToken 中的用户 ID 查询登录会话快照，恢复当前操作人</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
    ...
    <tr>
      ...
      <td>compile</td>
      <td>提供订单服务接口文档</td>
      <td>生成 <code>/order/v3/api-docs</code>，供网关 Swagger 聚合读取</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>compile</td>
      <td>服务注册与发现</td>
      <td>将 <code>order-service</code> 注册到 Nacos <code>zhou6</code> 命名空间</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>启动时加载 <code>common-shared.yml</code> 和 <code>order-service-dev.yml</code></td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
    ...
    <tr>
      ...
      <td>test</td>
      <td>测试基础能力</td>
      <td>供后续订单状态机、事务同步器和 Feign 调用测试使用</td>
      <td>当前使用</td>
    </tr>
    <tr>
      <td>order-service</td>
      ...
      <td>build plugin</td>
      <td>打包可执行服务</td>
      <td>将 <code>order-service</code> 打包成可执行 jar</td>
      <td>当前使用</td>
    </tr>
  </tbody>
</table>
...
<table border="1" cellspacing="0" cellpadding="6">
  ...
  <tbody>
    <tr>
      <td><code>a_doc/maven/order-service/order-service_2026年06月06日.md</code></td>
      <td>订单服务初始化与健壮性修复流水</td>
    </tr>
  </tbody>
</table>
